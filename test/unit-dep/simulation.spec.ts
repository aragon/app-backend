...
describe.only('SimulationController', () => {
  ...
  describe('simulateBundle', () => {
    it('should simulate valid actions successfully', async function() {
      ...
      daoFindStub.returns({
        lean: sinon.stub().resolves([{ address: '0x5afEb7F3259A25EB21287e3A917BeE3d4dE58dAf', network: NetworksEnum.ethereumMainnet }]),
      })

      pluginFindStub.returns({
        lean: sinon.stub().resolves([{ address: '0x18371E70D7c0cD13E4fD1356d3140B35301455d0',network: NetworksEnum.ethereumMainnet }]),
      })
