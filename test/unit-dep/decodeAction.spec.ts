...
describe('Integ: decodeAction', () => {
  ...
  describe('decode nested execute / createProposal actions', () => {
    it('should decode an execute action and its nested transfer action', async () => {
      // Real polygon-mainnet `execute(bytes32,(address,uint256,bytes)[],uint256)` carrying a
      // single nested `transfer(address,uint256)` action.
      const action = {
        to: '0x60bAbb3ee2Ebcf74e4b7397ae74f3B4BD2a9606e',
        value: '0',
        data: '',
      }
      ...
      const decoded: any = await decodeAction.decodeData(action, {
        ...
        daoAddress: '0x60bAbb3ee2Ebcf74e4b7397ae74f3B4BD2a9606e',
        blockNumber: 76328044,
      })

      expect(decoded?.type).to.be.eq(ProposalActionType.Execute)
      expect(decoded?.inputData?.function).to.be.eq('execute')
      expect(decoded?.inputData?.actions).to.have.length(1)

      const nested = decoded?.inputData?.actions?.[0]
      expect(nested?.type).to.be.eq(ProposalActionType.Transfer)
      expect(nested?.inputData?.function).to.be.eq('transfer')
      expect(nested?.to).to.be.eq('0x60bAbb3ee2Ebcf74e4b7397ae74f3B4BD2a9606e')
    })

    it('should decode a createProposal action and its nested action', async () => {
      // Real polygon-mainnet Multisig `createProposal(bytes,(address,uint256,bytes)[],...)`
      // carrying a single nested action.
      const action = {
        to: '0xDf35416389Fd28BC735bA8a8646ae4C9b26c2DB4',
        value: '0',
        data: '',
      }
      ...
      const decoded: any = await decodeAction.decodeData(action, {
        network: NetworksEnum.polygonMainnet,
        daoAddress: '0xCD0C7E6De1cc78881Fc7658f46dEf675B492A21D',
        blockNumber: 62151748,
      })

      expect(decoded?.type).to.be.eq(ProposalActionType.CreateProposal)
      expect(decoded?.inputData?.function).to.be.eq('createProposal')
      expect(decoded?.inputData?.actions).to.have.length(1)
      expect(decoded?.inputData?.actions?.[0].to).to.be.eq('0x6E9732b84DEfd8E8c591b4f9C03C700602Ff1DD7')
    })
  })
})
