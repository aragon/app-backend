...
describe('RouterV2: Token', () => {
  ...
  describe('getTokenStats', () => {
    it('Should getTokenStats', async () => {
      const params = {
        network: NetworksEnum.ethereumMainnet,
        address: '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2',
      }

      const mockStats = { holders: 100, transfers: 500 }
      const stubCtrl = sandbox.stub(TokenController, 'getTokenStats').resolves(mockStats)
      ...
      await TokenRouter.getTokenStats(ctx)

      expect(ctx.body).to.deep.equal(mockStats)
      expect(stubCtrl.calledOnce).to.be.true
      expect(stubCtrl.args[0]?.[0]).to.deep.eq({
        address: getAddress(params.address),
        network: params.network,
    ...
    it('Should handle different network values', async () => {
      ...
      const address = '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2'
      const stubCtrl = sandbox.stub(TokenController, 'getTokenStats')

      for (const network of networks) {
        stubCtrl.reset()
        const mockStats = { holders: 100, transfers: 500, network }
        stubCtrl.resolves(mockStats)
