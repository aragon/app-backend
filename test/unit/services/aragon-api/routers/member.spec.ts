...
describe('Router: Member', () => {
  ...
  describe('getHistoryMembersWithPagination', async () => {
    it('Should get member with pagination - all params', async () => {
      ...
      await MemberRouter.getHistoryMembersWithPagination(ctx)
    ...
    it('Should get member with pagination - daoId', async () => {
      ...
      expect(stubCtrl.args[0][0]).to.deep.eq({ ...paginationParams, ...missingParams })
      expect(stubCtrl.args[0][1]).to.deep.eq({
        onlyActive: filterParams.onlyActive,
        network: filterParams.network,
        daoAddress: undefined,
        pluginAddress: undefined,
        tokenAddress: undefined,
  ...
  it('Should getHistoryMemberByAddress using address', async () => {
    const params = {
      address: '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2',
    }
    ...
    await MemberRouter.getHistoryMemberByAddress(ctx)
