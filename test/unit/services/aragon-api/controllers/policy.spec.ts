...
describe('Controller: Policy', () => {
  ...
  describe('getPoliciesByDao', () => {
    it('should get policies for a DAO', async () => {
      ...
      const findByAddressStub = sandbox.stub(Models.Dao, 'findByAddress').resolves({ linkedAccounts: [] } as any)
    ...
    it('should include linked accounts when dao has linked accounts and onlyParent is false', async () => {
      ...
      sandbox.stub(Models.Dao, 'findByAddress').resolves({ linkedAccounts } as any)
      const findPoliciesStub = sandbox.stub(Models.Plugin, 'findPoliciesByDao').resolves(mockPolicies as any)
      ...
      expect(calledParams.daoAddresses).to.deep.eq(['0xDaoAddress', '0xLinkedAccount1', '0xLinkedAccount2'])
    })
