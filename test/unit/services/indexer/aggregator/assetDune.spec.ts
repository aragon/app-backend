...
import { AggregatorDuneAssets } from '@services/indexer/aggregator/assetDune'
...
describe('Indexer:Aggregator:AssetsDune', () => {
  ...
  it('should start the AggregatorDuneAssets', async () => {
    ...
    await AggregatorDuneAssets.start()

    expect(stubLogger.calledWith('End AggregatorDuneAssets' as any)).to.be.true
  ...
  describe('onDocument', async () => {
    it('should call onDocument and create asset', async () => {
      ...
      await AggregatorDuneAssets.onDocument(document as any)
    ...
    it('should call onDocument and update asset', async () => {
      ...
      const stubDune = sandbox.stub(DuneHelper, 'getBalance').resolves(fakeResp as any)
      const stubLogger = sandbox.spy(Logger, 'verbose')
      await AggregatorDuneAssets.onDocument(assetDb as any)
