...
describe('Indexer: PluginSettingHandler', () => {
  ...
  describe('votingSettingsUpdated', () => {
    ...
    it('should set setting as active when it has votingEscrow even if token is not governance', async () => {
      const parsedEvent = {
        args: {
          votingMode: 2n,
          supportThreshold: 150n,
      ...
      const stubFindExistingLog = sandbox.stub(Models.Setting, 'findExistingLog').resolves(false)
      const stubFindActive = sandbox.stub(Models.Setting, 'findActive').resolves(null)
      const getBlockTimestampStub = sandbox.stub(Web3Helper, 'getBlockTimestamp').resolves(123123123)

      // Token is NOT governance, but should still call isSupported because of votingEscrow
      const saveAndGetTokenStub = sandbox.stub(ProxyToken, 'saveAndGetToken').resolves({
        type: ITokenType.ERC20,
        isGovernance: false, // Not governance token
      } as any)
      ...
      const stubFindOne = sandbox.stub(Models.Plugin, 'findOne').resolves(null) // No SPP plugin

      await PluginSettingHandler.votingSettingsUpdated(parsedEvent as any, info as any)

      // Verify the setting was created with active status and votingEscrow data
      expect(createDocumentStub.calledOnce).to.be.true
      const createdSetting = createDocumentStub.firstCall.args[1]
      expect(createdSetting.status).to.equal(ISettingStatus.active)
      expect(createdSetting.votingEscrow).to.deep.equal({
        minDeposit: '100',
        minLockTime: 3600,
        cooldown: 3600,
        maxTime: 7200,
      ...
      // Verify votingEscrow settings were fetched
      expect(votingEscrowSettingsStub.calledOnce).to.be.true

      // Verify isSupported was called even though token is not governance
      // This happens because settingLog.votingEscrow exists
      expect(isSupportedStub.calledOnce).to.be.true
