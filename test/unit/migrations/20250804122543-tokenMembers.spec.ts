...
describe('migration: tokenMembers', () => {
  ...
  describe.only('check why we did not have the proper data', () => {
    it('should simulate complete migration with mock data arrays', async () => {
      ...
      await mongoose.connection.collection('MemberBalance').insertMany(mockMemberBalanceData.default || mockMemberBalanceData)
      await mongoose.connection.collection('MemberMetric').insertMany(mockMemberMetricsData.default || mockMemberMetricsData)
      await mongoose.connection.collection('MemberTransaction').insertMany(mockMemberTransactionData.default || mockMemberTransactionData)
      ...
      const totalMembers = await Models.Dao.countUniqueMembersCount(
        dao.address, dao.network
      )
