...
describe('Helpers:Web3', () => {
  ...
  describe('ensSubdomainExists', () => {
    it('should check if ensSubdomainExists', async () => {
      const stubConfigState = {
        getConfigItem: sandbox.stub().returns({}),
      }
      ...
      const result = await MockedWeb3Helper.ensSubdomainExists(ensName, NetworksEnum.ethereumMainnet)
    ...
    it('should log an error if checking ENS existence fails', async () => {
      ...
      expect(stubLoggerWarn.calledWith('Error ensSubdomainExists' as any)).to.be.true
    })
    ...
    it('should return false if not supported', async () => {
      ...
      const ensName = 'aavegotchi.dao.eth'
      const result = await MockedWeb3Helper.ensSubdomainExists(ensName, NetworksEnum.arbitrumMainnet)
