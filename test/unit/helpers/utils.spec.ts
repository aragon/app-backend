...
describe('Helpers:Utils', () => {
  ...
  describe('asyncBatchProcess', () => {
    it('should process all items successfully with default options', async () => {
      ...
      expect(processedItems).to.deep.equal([2, 4, 6, 8, 10])
    ...
    it('should handle custom concurrency and batch size', async () => {
      ...
      expect(processedItems).to.have.length(20)
    ...
    it('should collect errors without stopping processing by default', async () => {
      ...
      const onError = sandbox.stub().callsFake((error: Error, item: number) => {
      ...
      await Utils.asyncBatchProcess(items, processor, { onError })

      expect(processedItems).to.deep.equal([2, 6, 10]) // Only successful items
    ...
    it('should stop on first error when stopOnError is true', async () => {
      ...
      expect(processedItems).to.include.members([2, 4])
    ...
    it('should work with different data types', async () => {
      ...
      expect(processedItems).to.have.length(3)
      expect(processedItems[0]).to.deep.equal({ id: 1, name: 'Item 1', processed: true })
    })
    ...
    it('should handle async processor with varying execution times', async () => {
      ...
      expect(processedItems).to.have.length(5)
      expect(processor.callCount).to.equal(5)
