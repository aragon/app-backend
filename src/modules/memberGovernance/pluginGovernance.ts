...
export class PluginGovernance extends BaseGovernance {
  ...
  async create(memberAddress: HexAddress, params: IGovernanceParamsOpts): Promise<any> {
    ...
    try {
      return await DbTx.executeTxFn(async ({ session }) => {
        ...
        const plugin = await this.getPlugin(session)
        const pluginMember = await Models.PluginMember.create(
          {
            memberAddress: parsedAddress,
            pluginAddress: this.address,
            daoAddress: plugin?.daoAddress,
      ...
      logger.error('Error creating PluginMember', this.llo({ error, memberAddress: parsedAddress }))
      return null
    }
  }
  ...
  async delete(memberAddress: HexAddress): Promise<boolean> {
    ...
    try {
      return await DbTx.executeTxFn(async ({ session }) => {
        const pluginMember = await Models.PluginMember.findOne(
          {
            memberAddress: parsedAddress,
            pluginAddress: this.address,
            network: this.network,
