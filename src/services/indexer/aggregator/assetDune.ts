...
const llo = logger.logMeta.bind(null, { service: 'indexer:aggregator:AggregatorDuneAssets' })

export const AggregatorDuneAssets = {
  start: async () => {
    logger.verbose('Start AggregatorDuneAssets', llo({}))

    const aggregatorDb = await Models.Aggregator.findByType(AggregatorTypeEnum.assets)

    const crawler = new DBCrawler({
      model: Models.Dao,
      onDocument: AggregatorDuneAssets.onDocument,
      onError: (error: any) => {
        logger.error('Error AggregatorDuneAssets', llo({ error }))
      },
      where: {},
      batchSize: 500,
      concurrency: 1,
    ...
    logger.verbose('End AggregatorDuneAssets', llo({}))
  },
