...
import { ITelegramNotificationEvent, ITelegramSubscriptionStatus, TELEGRAM_DEFAULT_EVENTS } from '@types'
...
const HEADER = '*Your DAOs* — tap 🔔 to mute/unmute, ❌ to unsubscribe:'
...
export class DaoCommands extends BaseCommand {
  ...
  async list(ctx: BotContext): Promise<void> {
    ...
    if (!sub || sub.subscriptions.length === 0) {
      await ctx.reply(NO_DAOS_HEADER, {
        parse_mode: 'MarkdownV2',
        reply_markup: this.buildEmptyKeyboard(),
      })
      return
    }

    const rows = await this.enrichSubs(sub.subscriptions)
    await ctx.reply(HEADER, { parse_mode: 'MarkdownV2', reply_markup: this.buildKeyboard(rows) })
  }
  ...
  private async pause(ctx: BotContext): Promise<void> {
    ...
    if (!sub) {
      await ctx.reply('Nothing to pause — run /start first\\.', { parse_mode: 'MarkdownV2' })
      return
    }
    await sub.setStatus(ITelegramSubscriptionStatus.Paused)
    await ctx.reply('⏸ Notifications paused\\. Run /resume to turn them back on\\.', { parse_mode: 'MarkdownV2' })
  }
  ...
  private async resume(ctx: BotContext): Promise<void> {
    ...
    const sub = userId ? await Models.TelegramSubscription.findByTelegramUserId(userId) : null
    if (!sub) {
      await ctx.reply('Nothing to resume — run /start first\\.', { parse_mode: 'MarkdownV2' })
      return
    }
    await sub.setStatus(ITelegramSubscriptionStatus.Active)
    await ctx.reply('▶️ Notifications resumed\\.', { parse_mode: 'MarkdownV2' })
  }
  ...
  private async callback(ctx: BotContext): Promise<void> {
    ...
    switch (action) {
      case 'm': {
        const existing = sub.subscriptions.find((s: IRawSub) => s.daoId === daoId)
      ...
      case 'o': {
        ...
        const name = MarkdownV2.escape(dao?.name || daoId)
        const muted = sub.subscriptions.find((s: IRawSub) => s.daoId === daoId)?.events.length === 0
        await ctx.answerCallbackQuery(`Active: ${dao?.name ?? daoId}`)
        await ctx
          .reply([`*${name}*`, '', muted ? '🔕 Notifications are muted\\.' : '🔔 Notifications are on\\.'].join('\n'), {
            parse_mode: 'MarkdownV2',
          })
          .catch(() => undefined)
        return
  ...
  private async refreshKeyboard(ctx: BotContext): Promise<void> {
    ...
    if (!sub || sub.subscriptions.length === 0) {
      await ctx
        .editMessageText(NO_DAOS_HEADER, { parse_mode: 'MarkdownV2', reply_markup: this.buildEmptyKeyboard() })
        .catch(() => undefined)
      return
    }
    const rows = await this.enrichSubs(sub.subscriptions)
  ...
  private async enrichSubs(subs: IRawSub[]): Promise<ISubscriptionRow[]> {
    if (subs.length === 0) return []
    return await Promise.all(
      subs.map(async (s: IRawSub) => {
