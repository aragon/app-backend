...
export const DaoTransactions = {
  start: async ({ daoAddress, network }: { daoAddress: HexAddress; network: NetworksEnum }) => {
    try {
      ...
      if (SubscanApi.isPeaqNetwork(daoDb.network)) {
        await DaoTransactions.onDocumentForPeqaNetwork(daoDb)
      } else {
  ...
  onDocumentForPeqaNetwork: async (dao: Dao) => {
    const transactions = await SubscanApi.getAssetTransfer(dao.address, dao.network)
    ...
    await Promise.all(
      transactions.map(async (tx: any) => {
        try {
          ...
          const txValue = tx.category === 'external' ? tx.value : ethers.formatUnits(tx.value, token?.decimals || 0)
          ...
          const rawTx: Partial<Transaction> = {
            ...
            blockTimestamp,
            network: dao.network,
            type: tx.from === dao.address ? ITransactionType.withdraw : ITransactionType.deposit,
            daoAddress,
            pluginAddress,
            fromAddress: tx.from,
            toAddress: tx.to,
            value: Web3Helper.handleAlchemyCrazyBalance(txValue || 0, token?.decimals, tx),
            category: tx.category,
            proposalIndex,
          }
          const priceUsd = Number(token?.priceUsd || 0)
          rawTx.amountUsd = DaoTransactions.calculateAmountUsd(Number(txValue || 0), priceUsd)
