...
export const getContractInfo = async (network: NetworksEnum, address: string): Promise<IContractAbi | null> => {
  const mainData = await fetchVerifiedContractData(network, address)
  ...
  if (isProxy) {
    implementationData = await fetchVerifiedContractData(network, implementationAddress)
  }
...
async function fetchVerifiedContractData(
  ...
  return {
    ...
    functions: parseContractAbi(parsed),
  }
}

function parseContractAbi(abiResult: any[]) {
  return abiResult
    .filter(
      fn =>
        fn.type === 'function' &&
